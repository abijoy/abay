...
def dashboard(request):
    ...
    if request.user.is_authenticated:
        products_by_user = Product.objects.filter(
            created_by=request.user).order_by('-creation_date')
    ...
    context = {'user': user, 'products_by_user': products_by_user,
               'page_obj': page_obj, 'curr_datetime': curr_datetime}
...
def product_detail(request, id):
    ...
    logged_in_user_bid = Auction.objects.filter(
        product=p, placed_by=request.user).first()
    ...
    if p.auc_end_time < local_datetime:
        biggest_bid = Auction.objects.filter(
            product=p).aggregate(Max('amount'))
...
#                 'product_id': id
#             }
#         return render(request, 'app/edit_product.html', context=ctx)
#     return redirect(reverse('app:product_detail', kwargs={'id': p.id}))
...
def product_edit(request, id):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        if request.method == 'POST':
            ...
            images_to_be_added = request.FILES.getlist('images_to_be_added')
            images_to_be_removed = request.POST.getlist('images_to_be_removed')
            print(images_to_be_added)
            print(images_to_be_removed)
            ...
            if product_form.is_valid():
                ...
                try:
                    # first remove the requested existing images if any
                    for image_id in images_to_be_removed:
                        i = ProductImage.objects.filter(id=image_id).first()
                        i.product = None
                        i.save()

                    # Now add the new/updated images
                    for photo in images_to_be_added:
                        i = ProductImage.objects.create(image=photo)
                        i.product = product
                        i.save()
                except Exception as e:
                    print(e)
            ...
            data = {
                'message': 'SUCCESS',
                'success_url': f'/product/detail/{product.id}/'
            }
            return JsonResponse(data)
    ...
    return render(request, 'app/edit_product.html', context=context)
...
def bids(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        if request.method == 'POST':
            ...
            existed_bid = Auction.objects.filter(
                product=product, placed_by=request.user).first()
            highest_bid = Auction.objects.filter(
                product=product).order_by('-amount').first()
...
def user_dashboard(request):
    products_by_user = Product.objects.filter(
        created_by=request.user).order_by('-auc_end_time', '-creation_date')
    bidded_items = Auction.objects.filter(
        placed_by=request.user).order_by('product')
